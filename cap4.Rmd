...
La División Estadística de la FAO (FAOSTAT) tiene publicada información sobre el número de colmenas por país desde el año 1961 y hasta el 2018, la cual se resume para nuestro país, en la tabla y gráfica siguientes: 
...
<p align="center"> **Cuadro 12. Colmenas en México 1961-2018.** [`r icon::fa("download")`](tabs/BeehivesMX.csv) </p>
...
dygraph(p_anual_colmenas, xlab ="Año") %>% 
  dySeries(c("V1"), label = "Número de Colmenas") %>%
...
<p align="center"> **Gráfica 1. Colmenas en México 1961-2017** [`r icon::fa("download")`](tabs/BeehivesMX.csv) </p>


***Fuente:***

[FAOSTAT. *Datos sobre animales vivos.* 2017.](http://www.fao.org/faostat/en/#data/QA){target="_blank"}
...
p1 <- plot_ly(bx3, x = ~years, y = ~AGS, name = 'Aguascalientes', type = 'scatter', mode = 'lines',
  ...
  layout(title = "Gráfica 2. Producción de Miel por Entidad Federativa, 2003-2019",
...
<br/>
*(Dar doble clic en la entidad federativa de interés para visualizarla de forma aislada).*  [`r icon::fa("download")`](tabs/ent_ton_miel_03_18.csv)
...
## 4.3. Producción de Miel por municipio 2018.
****
...
tabmun_18 <- read.csv("tabs/Prodmiel2018muni.csv", encoding = "UTF")
...
<p align="center"> **Cuadro 13. Producción de Miel por municipio, 2018** [`r icon::fa("download")`](tabs/Prodmiel2018muni.csv) </p>

<br/>

<br/>

## 4.4. Otras Estadísticas por Entidad Federativa.
****
...
asoc_ent_2 <- read.csv("tabs/estadisticas.csv", encoding="UTF")
...
p2 <- plot_ly(datos2a, x = ~datos2a$Entidad, y = ~datos2a$asoc_ent_2a.orga_asoc_apic, type = 'bar',
  ...
  layout(title = ("Gráfica 3. Asociaciones Apícolas por Entidad Federativa, 2017"),
...
estadisticas_t1 <- read.csv("tabs/estadisticas.csv", encoding="UTF")
...
grafica1 <- plot_ly(datos1, x = ~xestados_t1, y = ~yacopiador_t1, type = 'bar', marker = list(color = 'deepskyblue2'),
  ...
  layout(title = ("Grafica 4. Unidades Económicas Acopiadoras de Miel por Entidad Federativa, 2017"),
...
estadisticas_t2 <- read.csv("tabs/estadisticas.csv", encoding="UTF")
...
grafica2 <- plot_ly(datos2, x = ~xestados_t2, y = ~yexportador_t2, type = 'bar', marker = list(color = 'deepskyblue2')) %>%
  layout(title = ("Gráfica 5. Unidades Económicas Exportadoras de Miel por Entidad Federativa, 2017"),
...
```{r graf_b4, fig.align='center', fig.height= 6, fig.width= 12, echo=FALSE, out.width = "100%", cache=TRUE, warning=FALSE}

criad_reinas <- read.csv("tabs/estadisticas.csv", encoding="UTF")
criad_reinas1<- subset(criad_reinas, criad_reinas[,7]>0 | criad_reinas[,8]>0 | criad_reinas[,9]>0 )
criad_reinas2 <- data.frame(criad_reinas1$Entidad, criad_reinas1[,7], criad_reinas1[,8], criad_reinas1[,9])   
criad_reinas2$x<- factor(criad_reinas2$criad_reinas1.Entidad, levels = criad_reinas2[["criad_reinas1.Entidad"]])


library(plotly)

p3 <- plot_ly(criad_reinas1, x = ~criad_reinas2[,5], y = ~criad_reinas2[,2], type = 'bar',
             name = 'Criadores de <br> Abeja Reina', marker = list(color = 'rgb(49,130,189)')) %>%
  add_trace(y = ~criad_reinas2[,3], name = 'Criadores de <br> Abeja Reina <br>  Progenitoras', 
            marker = list(color = 'green')) %>% 
  add_trace(y = ~criad_reinas2[,4], name = 'Productores <br> de Núcleos <br> o Paquetes <br> de Abejas', 
            marker = list(color = 'red')) %>% 
  layout(title = ("Gráfica 6. Criadores de Abeja Reina y Núcleos, por Entidad Federativa (2017)"),
         xaxis = list(title = "", tickangle =-45),
         yaxis = list(title = "Criadores/Productores"),
         margin = list(b = 100),
         barmode = 'group')
p3
...
```{r graf_b5, fig.align='center', fig.height= 6, fig.width= 12, echo=FALSE, out.width = "100%", cache=TRUE, warning=FALSE}

abejas_reina <- read.csv("tabs/estadisticas.csv", encoding="UTF")
abejas_reina1<- subset(criad_reinas, criad_reinas[,10]>0 | criad_reinas[,11]>0 | criad_reinas[,12]>0 | criad_reinas[,18]>0 )
abejas_reina2 <- data.frame(criad_reinas1$Entidad, criad_reinas1[,10], 
                              criad_reinas1[,11], criad_reinas1[,12], criad_reinas1[,18])   
abejas_reina2$x<- factor(abejas_reina2$criad_reinas1.Entidad, levels = abejas_reina2[["criad_reinas1.Entidad"]])

library(plotly)

p4 <- plot_ly(abejas_reina2, x = ~abejas_reina2[,6], y = ~abejas_reina2[,2], type = 'bar',
              name = 'Abejas Reina <br> Certificadas', marker = list(color = 'rgb(49,130,189)')) %>%
  add_trace(y = ~abejas_reina2[,3], name = 'Abejas Reina <br> No Certificadas', 
            marker = list(color = 'green')) %>% 
  add_trace(y = ~abejas_reina2[,4], name = 'Núcleos <br> Producidos', 
            marker = list(color = 'red')) %>% 
  add_trace(y = ~abejas_reina2[,5], name = 'Colmenas para <br> Polinización', 
            marker = list(color = 'orange')) %>% 
  layout(title = ("Gráfica 7. Abejas Reina, Núcleos y Colmenas para Polinización por Entidad Federativa (2017)"),
         xaxis = list(title = "", tickangle =-45),
         yaxis = list(title = "Cantidad producida"),
         margin = list(b = 100),
         barmode = 'group')
p4
...
grafica3 <- plot_ly(datos3, x = ~xestados_t, y = ~ypeso_cera_t, type = 'bar',
  ...
  layout(title = ("Gráfica 8. Producción de Cera por Entidad Federativa, 2017 (ton)"),
         xaxis = list(title = "", tickangle = -45),  
         size = 16,
...
```
<p align="center"> *Fuente: Sistema de Información Agroalimentaria de Consulta (SIACON), 2018.* [`r icon::fa("download")`](tabs/prodcera2018.csv) </p>
...
miel_org <- read.csv("tabs/estadisticas.csv", encoding="UTF")
...
p5 <- plot_ly(miel_org2, x = ~miel_org2$x, y = ~miel_org2[,13], type = 'bar',
  ...
  layout(title = ("Gráfica 9. Producción de Miel Orgánica por Entidad Federativa, 2017 (Ton)"),
         xaxis = list(title = "", tickangle =-45),
...
derivados <- read.csv("tabs/estadisticas.csv", encoding="UTF")
...
p6 <- plot_ly(derivados2, x = ~derivados2[,5], y = ~derivados2[,2], type = 'bar',
  ...
  add_trace(y = ~derivados2[,4], name = 'Propóleos', 
            marker = list(color = 'red')) %>% 
  layout(title = ("Gráfica 10. Producción de Derivados Apícolas por Entidad Federativa, 2017"),
         xaxis = list(title = "", tickangle =-45),
         yaxis = list(title = "kg"),
         margin = list(b = 100),
         barmode = 'group')
...
p7 <- plot_ly(terr2, x = ~terr2[,5], y = ~terr2[,2], type = 'bar',
  ...
  add_trace(y = ~terr2[,4], name = 'Terrenos Privados', 
            marker = list(color = 'red')) %>% 
  layout(title = ("Gráfica 11. Terrenos Dedicados a Actividades Apícolas, por Entidad Federativa (2017)"),
         xaxis = list(title = "", tickangle =-45),
...
p8 <- plot_ly(hect2, x = ~hect2[,5], y = ~hect2[,2], type = 'bar',
  ...
  add_trace(y = ~hect2[,4], name = 'Terrenos Privados', 
            marker = list(color = 'red')) %>% 
  layout(title = ('Gráfica 12. Superficie Dedicada a Actividades Apícolas, por Entidad Federativa, 2017'),
         xaxis = list(title = "", tickangle =-45),
...
##  4.5. Balanza Comercial de México. Importación y Exportación   
****
...
[SADER. *Nota de Prensa.* Agosto, 2018.](https://www.gob.mx/sader/prensa/se-incrementa-en-el-primer-semestre-del-ano-la-exportacion-de-miel-mexicana-en-mas-de-11-por-ciento?idiom=es){target="_blank"}
...
<p align="center"> **Cuadro 14. Exportaciones Mensuales de Miel, 1993-2018** [`r icon::fa("download")`](tabs/3_16_1_exportamiel.csv) </p>
...
<p align="center"> **Gráfica 13. Exportaciones Mensuales de Miel, 1993-2018** [`r icon::fa("download")`](tabs/3_16_1_exportamiel.csv) </p>
...
[BANXICO. Sistema de Información Económica. *Exportación de Productos Agropecuarios (Miel).* 2018.](http://www.banxico.org.mx/SieInternet/consultarDirectorioInternetAction.do?accion=consultarCuadro&idCuadro=CE122&locale=es){target="_blank"}
...
<p align="center"> **Descargar datos** [`r icon::fa("download")`](tabs/import_export.csv) </p>

<br/>
***Fuente:***

[INEGI. *Estadísticas de Comercio Exterior*. 2018.](https://www.inegi.org.mx/datos/?t=0030000000000000){target="_blank"}
...
immiel <- read.csv("tabs/import_2004_2018.csv", encoding = "UTF")

importac <- plot_ly(immiel, x = ~anio, y = ~dolares, type = 'scatter', 
              mode = 'lines', line = list(color = 'darkblue', 
                                          width = 4))  %>%
  layout(title = "Gráfica 15. Importaciones Anuales de Miel, 2004-2018",
         xaxis = list(title = "Año"),
         yaxis = list (title = "Dólares"))
importac
